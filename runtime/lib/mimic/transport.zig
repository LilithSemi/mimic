...
pub const Device = struct {
    ...
    /// Words per streaming burst. 240 * 4 = 960 bytes, roughly 1KB with the
    /// header, matching the write burst budget.
    ...
    /// Reads `words.len` words from the FIFO at `addr`, where every read
    /// pops the same register.
    ///
    /// The READ opcode advances the address one word per word inside a
    /// frame, so a burst cannot walk a FIFO. Each word therefore costs one
    /// frame of its own. A record of a few words is worth that, and the
    /// protocol needs no new opcode for it.
